$(function() {
    ...
    var TIMEMAINTPL = '<div class="c-datepicker-time-panel c-datepicker-popper" style="">' +
        ...
        '<button type="button" class="c-datepicker-time-panel__btn min">0點</button>' +
        ...
        '<button type="button" class="c-datepicker-time-panel__btn confirm">確定</button>' +
        '</div>' +
        '</div>';
    ...
    var PICKERTIMEHEADERTPL = '<span class="{{className}}__editor-wrap">' +
        ...
        '<input type="text" autocomplete="off" placeholder="選擇日期" class="c-datepicker-input__inner c-datePicker__input-day">' +
        '</div>' +
        '</span>' +
        '<span class="{{className}}__editor-wrap">' +
        '<div class="c-datepicker-input c-datepicker-input--small">' +
        '<input type="text" autocomplete="off" placeholder="選擇時間" class="c-datepicker-input__inner c-datePicker__input-time">' +
        '</div>' +
        '</span>';
    //  範圍
    ...
    $.extend(Time.prototype, {
        ...
        event: function() {
            ...
            this.picker.$container.find('.c-datepicker-scrollbar__wrap').scroll(function() {
                ...
                // 兼容滚动延時，导致下一个時間滚动清除掉上一个的timer,所以分开是三个
        ...
        event: function() {
            ...
            this.pickerObject.$input.on('blur', function() {
                ...
                if (_this.pickerObject.hasTime) {
                    ...
                    // 邏輯：先blur-绑定重新渲染date panel事件-触發選擇日期或年月切换等事件-發布重新渲染事件|取消發布事件
                    // 觸理的类型有，點擊切换上下一年月，點擊年月選擇，選擇日期，點擊piker其他

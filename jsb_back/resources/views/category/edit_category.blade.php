...
<div class="bg-white p-3">
        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">編輯主分類</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <form action="{{route('updateMainCategory')}}" method="POST" class="p-2">
    {{ csrf_field() }}
        <div class="p-2">
        <div class="p-2">
        <div class="my-4 row  align-items-center g-0">
            <h3 class="col-xxl-1 col-md-2 col-12 fs-6 text-center text-start m-md-0 mb-1">主分類名稱</h3>
            <div class="col-md-10 col-12">
                <input class="form-control " type="hidden" placeholder="分類名稱" name="cateMainId" required value="{{$cateMain[0]->id}}">
                <input class="form-control " type="text" placeholder="分類名稱" name="cateMainName" required value="{{$cateMain[0]->cateMainName}}">
            </div>
        </div>
        <div class="my-4 row   align-items-center g-0">
            <h3 class="col-xxl-1 col-md-2 col-12 fs-6 text-center text-start m-md-0 mb-1">啟用</h3>
            <label class="switch">
                        @if($cateMain[0]->enable == 1)
        ...
        <div class="my-4 row   align-items-center g-0">
            ...
            <div class="col-md-10 col-12">
                <select class="form-select" aria-label="Default select example" name="sort">
                    <?for($i=1;$i<=50;$i++){?>
                        <option value="<?=$i?>" <?if($cateMain[0]->sort==$i){?>selected<?}?>><?=$i?></option>
                    <?}?>
                </select>
            </div>
        </div>

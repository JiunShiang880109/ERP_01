               ...
               <div class="ps-3">
                   <nav aria-label="breadcrumb">
                       <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">修改常用傳票</li>
                       </ol>
                   </nav>
               </div>
           </div>
        ...
        <div class="content-wrap mt-3">
            <div class="main">
                <div class="card alert ">
                        <div class="card-body">
                            {{--{{dd(session()->all())}}--}}
                            <form action="{{route('accountant.update_voucher_temp', $voucher_temp->id)}}" method="POST">
                                @csrf
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        class="form-control"
                                        value="{{old('voucher_date', optional($voucher_temp->voucher_date)->format('Y-m-d'))}}">
                                    </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select" name="voucher_type">
                                            <option value="0"
                                                {{ old('voucher_type', $voucher_temp->voucher_type) == 0 ? 'selected' : '' }}>
                                                現金收入
                                            </option>
                                            <option value="1"
                                                {{ old('voucher_type', $voucher_temp->voucher_type) == 1 ? 'selected' : '' }}>
                                                現金支出
                                            </option>
                                            <option value="2"
                                                {{ old('voucher_type', $voucher_temp->voucher_type) == 2 ? 'selected' : '' }}>
                                                轉帳
                                            </option>
                                        </select>
                                        <input type="hidden" name="voucher_type" value="{{ $voucher_temp->voucher_type }}">
                                    </div>

                                    <div class="col-md-12">
                                        <label class="form-label">傳票摘要</label>
                                        <input type="text" name="note" class="form-control"
                                                value="{{ old('note', $voucher_temp->note) }}">
                                    </div>
                                </div>
                                <hr>
                                <h6>分錄明細</h6>
                                ...
                                @foreach($voucher_temp->items as $i => $row)
                                    <div class="row g-3 item-row">
                                        ...
                                        {{-- 科目 --}}
                                        <div class="col-md-2">
                                            <select class="form-select subject-selecte" name="items[{{ $i }}][ledger_code]">
...
<script>
    ...
    $(document).on('change', 'select[name$="[ledger_code]"]', function () {
    ...
    let index= {{ $voucher_temp->items->count() }};
    ...
    //更新常用分錄
    $('#update-temp-btn').on('click', function(){
        const form = $('form');
        const formData = form.serialize() + '&action=update_temp';

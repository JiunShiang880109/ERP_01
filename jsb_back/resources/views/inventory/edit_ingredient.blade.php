               ...
               <div class="ps-3">
                   <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">修改項目</li>
                        </ol>
                   </nav>
               </div>
           </div>
        ...
        <div class="content-wrap mt-3">
            <div class="main">
                <div class="card alert ">
                        <div class="card-body">
                            <form action="{{route('inventory.update', $ingredients->id)}}" enctype="multipart/form-data" method="POST" class="row g-3">
                                @csrf

                                {{-- Title --}}
                                <div class="card-title d-flex align-items-center">
                                    <div><i class="bx bxs-edit me-1 font-22 text-primary"></i></div>
                                    <h5 class="mb-0 text-primary">修改原物料</h5>
                                </div>
                                <hr>
                                        ...
                                        <div class="col-lg-4">
                                            <img id="preview"
                                                src="{{ $ingredients->imageUrl 
                                                        ? asset('assets/images/ingredients/' . $ingredients->imageUrl)
                                                        : asset('assets/images/avatars/avatar-0.png') }}"
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="name" class="form-control" value="{{$ingredients->name}}" required>
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select" name="categoryMainId" required>
                                        ...
                                        @foreach($ingredientsCateMain as $ic)
                                            <option value="{{$ic->id}}"
                                                {{$ingredients->categoryMainId == $ic->id ? 'selected' : ''}}>
                                                {{$ic->name}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="unit" class="form-control" value="{{ $ingredients->unit }}" required>
                                </div>

                                {{-- 安全庫存 --}}
                                <div class="col-md-4">
                                    <label class="form-label">安全庫存</label>
                                    <input type="number" step="1" name="safeAmount" class="form-control" value="{{$ingredients->safeAmount}}" required>
                                </div>

                                {{-- 目前庫存 --}}
                                <div class="col-md-4">
                                    <label class="form-label">目前庫存（預設 0）</label>
                                    <input type="number" step="1" name="stockAmount" class="form-control" value="{{$ingredients->stockAmount}}">
                                </div>

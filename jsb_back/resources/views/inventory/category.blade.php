        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">庫存成本類別</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="modal fade" id="AddUnit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{route('inventory.category_add')}}" method="POST">
                    @csrf
                    {{--  --}}
                    <div class="m-4 row align-items-center g-0">
                        <h3 class="col-md-2 col-12 fs-6 text-center text-start m-md-0 mb-1">類別名稱</h3>
                        <div class="col-md-10 col-12">
                            <input name="name" class="form-control " type="text" placeholder="ex:飲品原料、肉類、包材...">
                        </div>
                    </div>
                    {{-- 排序 --}}
                    <div class="m-4 row align-items-center g-0">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex order-actions">
                                        ...
                                        <form action="{{route('inventory.category_delete', $c->id)}}" method="POST" onsubmit="return confirm('確定要刪除該筆資料嗎?');" class="m-0">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="modal fade" id="editUnit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form id="editForm" method="POST" onsubmit="return confirm('確定要修改該筆資料嗎?');">
                    @csrf
                    {{--  --}}
                    <div class="m-4 row align-items-center g-0">
                        <h3 class="col-md-2 col-12 fs-6 text-center text-start m-md-0 mb-1">類別名稱</h3>
                        <div class="col-md-10 col-12">
                            <input id="edit_name" name="name" class="form-control " type="text" placeholder="ex:飲品原料、肉類、包材...">
                        </div>
                    </div>
                    {{-- 排序 --}}
                    <div class="m-4 row align-items-center g-0">
...
$(document).on('click', '.editBtn', function () {
    ...
    // 設定 form 的 action
    $('#editForm').attr('action', '/inventory/category/' + id + '/update');
});
</script>
@endsection

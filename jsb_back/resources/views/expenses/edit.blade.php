               ...
               <div class="ps-3">
                   <nav aria-label="breadcrumb">
                       <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">修改支出</li>
                       </ol>
                   </nav>
               </div>
           </div>
        ...
        <div class="content-wrap mt-3">
            <div class="main">
                <div class="card alert ">
                        <div class="card-body">
                            {{--{{dd(session()->all())}}--}}
                            <form action="{{route('expenses.update', $expense->id)}}" enctype="multipart/form-data"  method="POST" onsubmit="return confirm('確定要修改該筆資料嗎?');" class="row g-3">
                                 @csrf
                                 <div class="card-title d-flex align-items-center">
                                    <div><i class="bx bxs-edit me-1 font-22 text-primary"></i>
                                    </div>
                                    <h5 class="mb-0 text-primary">修改支出</h5>
                                </div>
                                <hr>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <!-- <input type="text" name="category_main" class="form-control" value="{{$expense->category_main}}"> -->
                                    <select name="category_main_id" class="form-select">
                                        @foreach($categoryMain as $cm)
                                            <option value="{{ $cm->id }}"
                                                @if($expense->category_main_id == $cm->id) selected @endif>
                                                {{ $cm->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="category_sub" class="form-control" value="{{$expense->category_sub}}">
                                </div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="payMethod" class="form-control" value="{{$expense->payMethod}}">
                                </div>
